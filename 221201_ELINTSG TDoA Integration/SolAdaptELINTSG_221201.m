...
% pri     = ([1:min_obs-1./diff(reported_toa{1}).'; 1:min_obs-1./diff(reported_toa{2}).'; 1:min_obs-1./diff(reported_toa{3}).'; 1:min_obs-1./diff(reported_toa{4}).'])./1e3;
...
figure; 
subplot(4,1,1); scatter(1:length(err_z1),vecnorm(emitter_pos - abs(sol1)),15,'filled'); title('Absolute Range Error'); xlabel('TOA Sample #'); ylabel('Error (meters)')
...
% figure; 
% plot(1:length(err_x2),err_x1, ...
%     1:length(err_y2),err_y1, ...
%     1:length(err_z2),err_z1, ...
%     1:length(err_z2),vecnorm(emitter_pos - sol2), ...
%     'LineWidth',1.5); title('Error in Solution "2"'); xlabel('TOA Sample #'); ylabel('Error (meters)')
% legend('Error in X','Error in Y','Error in Z', 'Absolute Range Error')

figure; 
subplot(4,1,1); plot(1:length(err_z1),vecnorm(emitter_pos - abs(sol2))); title('Absolute Range Error'); xlabel('TOA Sample #'); ylabel('Error (meters)')
subplot(4,1,2); plot(1:length(err_x1),abs(err_x2)); title('Error in X'); xlabel('TOA Sample #'); ylabel('Error (meters)')
subplot(4,1,3); plot(1:length(err_y1),abs(err_y2)); title('Error in Y'); xlabel('TOA Sample #'); ylabel('Error (meters)')
subplot(4,1,4); plot(1:length(err_z1),abs(err_z2),LineWidth=0.1); title('Error in Z'); xlabel('TOA Sample #'); ylabel('Error (meters)')
