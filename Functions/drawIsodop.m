function xy_iso = drawIsodop(x1,v1,x2,v2,sourceVel,vdiff,numPts,maxOrtho)
...
%   sourceVel   velocity of the emitter (Ndim x 1) [m/s]
...
df_plot = CalcDopDiff(x_plot,sourceVel,x1,v1,x2,v2,f_0);
    % numPts^2 x (N-1)
    
%% Compute contour
%% Check whether a trivial solution exists. In case of a trivial solution, the target is inherently undetectable
if all(isnan(df_plot))
    error('Solution does not exist for the current geometry')
else
    fig00=figure;
